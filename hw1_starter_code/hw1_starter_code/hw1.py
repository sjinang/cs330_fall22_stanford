...
class MANN(nn.Module):
    ...
    def forward(self, input_images, input_labels):
        ...
        labels = input_labels.detach().clone()
        labels[:,-1,:,:]=0  
        
        inp = torch.cat((labels.reshape((-1,self.num_classes*(self.samples_per_class),self.num_classes)),
                        input_images.reshape((-1,self.num_classes*(self.samples_per_class),784))),
                        dim=-1)

        out1, _ = self.layer1(inp)
        out, _ = self.layer2(out1)
        return out.view(-1,self.samples_per_class,self.num_classes,self.num_classes)
    ...
    def loss_function(self, preds, labels):
        ...
        return loss_fn(preds[:,-1].reshape((-1,self.num_classes)),torch.argmax(labels[:,-1],dim=-1).reshape((-1,)))
        #############################
